...
foreach ($target in $targets) {
    ...
    $salutation = if ($target.contact_name) { "Hi $($target.contact_name)," } else { "Hi," }
...
"@
    ...
    $fileName = "DRAFT_$($count)_$($target.company -replace '[^a-zA-Z0-9]', '_').txt"
    $fullBody | Out-File "drafts/$fileName"
    $count++
}

Write-Output "Generated $count high-intensity draft files in /drafts folder."
