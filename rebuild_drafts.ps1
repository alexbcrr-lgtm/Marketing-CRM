...
# Clear old drafts folder to prevent confusion
if (Test-Path "C:\Users\alexb\.openclaw\workspace\drafts") {
    Remove-Item -Path "C:\Users\alexb\.openclaw\workspace\drafts\*" -Force
} else {
    New-Item -ItemType Directory -Path "C:\Users\alexb\.openclaw\workspace\drafts"
}

foreach ($target in $targets) {
    # Define keywords for Furniture Dealers (Vector)
    $dealerKeywords = @("Interiors", "Furniture", "Furnishings", "Design Center", "Showroom", "Dealer", "Systems", "Herman Miller", "Steelcase", "Haworth", "Knoll", "Kimball", "Workspace", "Workscapes", "Ebanista")
    
    # Logic: If description or company contains dealer keywords, it's Vector. 
    # Otherwise, it's Business Moving (Brokers/Companies).
    $isDealer = $false
    foreach ($word in $dealerKeywords) {
        if ($target.company -like "*$word*" -or $target.description -like "*$word*") {
            $isDealer = $true
            break
        }
    }
    ...
    $safeName = $company.Replace(' ', '_').Replace('|', '_').Replace('/', '_').Replace('\', '_').Replace(':', '_')
    $fileName = "FROM_$($sender)_TO_$($safeName).txt"
...
We also know Southern California. The buildings, the loading docks, the freight elevators, the parking situations. That local knowledge makes a real difference on moving day.
...
Write-Output "Correctly filtered and generated $count drafts."
