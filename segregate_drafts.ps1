...
foreach ($target in $targets) {
    ...
    # Alternating to provide a mix for review
    $type = if ($count % 2 -eq 0) { "Installation" } else { "Moving" }
    $sender = if ($type -eq "Installation") { "alex@vectorinstallations.com" } else { "alex@businessmoving.com" }
...
Write-Output "Generated $count segregated drafts in \workspace\drafts\"
